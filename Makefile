...
GO_PACKAGES=./services/... ./indexer/... ./unelmacoin/... ./configuration/...
...
build:
	docker build -t rosetta-unelmacoin:latest https://github.com/unelmacoin/rosetta-unelmacoin.git

build-local:
	docker build -t rosetta-unelmacoin:latest .

build-release:
	# make sure to always set version with vX.X.X
	docker build -t rosetta-unelmacoin:$(version) .;
	docker save rosetta-unelmacoin:$(version) | gzip > rosetta-unelmacoin-$(version).tar.gz;

run-mainnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/unelmacoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=MAINNET" -e "PORT=8080" -p 8080:8080 -p 8333:8333 rosetta-unelmacoin:latest

run-mainnet-offline:
	docker run -d --rm -e "MODE=OFFLINE" -e "NETWORK=MAINNET" -e "PORT=8081" -p 8081:8081 rosetta-unelmacoin:latest

run-testnet-online:
	docker run -d --rm --ulimit "nofile=${NOFILE}:${NOFILE}" -v "${PWD}/unelmacoin-data:/data" -e "MODE=ONLINE" -e "NETWORK=TESTNET" -e "PORT=8080" -p 8080:8080 -p 18333:18333 rosetta-unelmacoin:latest
...
lint: | check-comments
	golangci-lint run --timeout 5m0s -v -E ${LINT_SETTINGS},gomnd
