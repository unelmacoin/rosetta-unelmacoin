...
type Client interface {
	...
	GetRawBlock(context.Context, *types.PartialBlockIdentifier) (*unelmacoin.Block, []string, error)
	ParseBlock(
		context.Context,
		*unelmacoin.Block,
...
// waitForNode returns once unelmacoind is ready to serve
// block queries.
func (i *Indexer) waitForNode(ctx context.Context) error {
	...
	for {
		...
		logger.Infow("waiting for unelmacoind...")
...
// Sync attempts to index unelmacoin blocks using
// the unelmacoin.Client until stopped.
...
// Prune attempts to prune blocks in unelmacoind every
// pruneFrequency.
func (i *Indexer) Prune(ctx context.Context) error {
	...
	for {
		...
		case <-tc.C:
			...
			logger.Infow("attempting to prune unelmacoind", "prune height", pruneHeight)
			prunedHeight, err := i.client.PruneBlockchain(ctx, pruneHeight)
			if err != nil {
				logger.Warnw(
					"unable to prune unelmacoind",
					"prune height", pruneHeight,
					"error", err,
				)
			} else {
				logger.Infow("pruned unelmacoind", "prune height", prunedHeight)
			}
		}
	}
}
...
func (i *Indexer) findCoin(
	...
	for ctx.Err() == nil {
		...
		// multiple listeners)
		transactionHash := unelmacoin.TransactionHash(coinIdentifier)
...
func (i *Indexer) findCoins(
	...
	for _, coinIdentifier := range remainingCoins {
		// Wait on Channel
		txHash := unelmacoin.TransactionHash(coinIdentifier)
...
func (i *Indexer) GetScriptPubKeys(
	ctx context.Context,
	coins []*types.Coin,
) ([]*unelmacoin.ScriptPubKey, error) {
	databaseTransaction := i.database.ReadTransaction(ctx)
	defer databaseTransaction.Discard(ctx)

	scripts := make([]*unelmacoin.ScriptPubKey, len(coins))
	for j, coin := range coins {
		coinIdentifier := coin.CoinIdentifier
		transactionHash, networkIndex, err := unelmacoin.ParseCoinIdentifier(coinIdentifier)
		if err != nil {
		...
		for _, op := range transaction.Operations {
			if op.Type != unelmacoin.OutputOpType {
				continue
			}
			...
			var opMetadata unelmacoin.OperationMetadata
