...
import (
	...
	mocks "github.com/unelmacoin/rosetta-unelmacoin/mocks/indexer"
...
func TestIndexer_Transactions(t *testing.T) {
	...
	type coinBankEntry struct {
		Script  *unelmacoin.ScriptPubKey
		Coin    *types.Coin
	...
	for i := int64(0); i <= 1000; i++ {
		...
		for j := 0; j < 5; j++ {
			...
			scriptPubKey := &unelmacoin.ScriptPubKey{
				ASM: coinIdentifier,
			}
				...
				Operations: []*types.Operation{
					{
						...
						Status: types.String(unelmacoin.SuccessStatus),
	...
	go func() {
		for {
			...
			if currBlock.BlockIdentifier.Index == 1000 {
				...
				allCoins := []*types.Coin{}
				expectedPubKeys := []*unelmacoin.ScriptPubKey{}
