...
var (
	...
	// ErrNotReady is returned when Unelmacoind is not
	// yet ready to serve queries.
	ErrNotReady = &types.Error{
		Code:      2, //nolint
		Message:   "Unelmacoind is not ready",
		Retriable: true,
	}

	// ErrUnelmacoind is returned when Unelmacoind
	// errors on a request.
	ErrUnelmacoind = &types.Error{
		Code:    3, //nolint
		Message: "Unelmacoind error",
	}
	...
	// Unelmacoin.ScriptPubKeys to construct a transaction.
	...
	// Unelmacoin.ScriptPubKey cannot be parsed during construction.
